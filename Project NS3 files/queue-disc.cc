...
QueueDisc::Stats::Stats ()
  : nTotalReceivedPackets (0),
	...
	nTotalReceivedTcpPackets (0),
	nTotalReceivedTcpBytes (0),
	nTotalReceivedUdpPackets (0),
	nTotalReceivedUdpBytes (0),

    nTotalSentPackets (0),
    nTotalSentBytes (0),
	nTotalFilteredPackets (0),
	nTotalFilteredBytes (0),
	nTotalFilteredTcpPackets (0),
	nTotalFilteredTcpBytes (0),
	nTotalFilteredUdpPackets (0),
	nTotalFilteredUdpBytes (0),

    nTotalDroppedPacketsBeforeEnqueue (0),
    nTotalDroppedBytesBeforeEnqueue (0),
	nTotalDroppedTcpPacketsBeforeEnqueue (0),
	nTotalDroppedTcpBytesBeforeEnqueue (0),
	nTotalDroppedUdpPacketsBeforeEnqueue (0),
	nTotalDroppedUdpBytesBeforeEnqueue (0),
...
QueueDisc::Stats::Print (std::ostream &os) const
{
  ...
  os << std::endl << "Packets/Bytes received: "
                  ...
                  << nTotalReceivedBytes
	 << std::endl << "TCP Packets/Bytes received: "
				  << nTotalReceivedTcpPackets << " / "
				  << nTotalReceivedTcpBytes
	 << std::endl << "UDP Packets/Bytes received: "
				  << nTotalReceivedUdpPackets << " / "
				  << nTotalReceivedUdpBytes

	 << std::endl << "Packets/Bytes filtered: "
				  << nTotalFilteredPackets << " / "
				  << nTotalFilteredBytes
	 << std::endl << "TCP Packets/Bytes filtered: "
				  << nTotalFilteredTcpPackets << " / "
			      << nTotalFilteredTcpBytes
	 << std::endl << "UDP Packets/Bytes filtered: "
				  << nTotalFilteredUdpPackets << " / "
				  << nTotalFilteredUdpBytes
	 ...
	 << std::endl << "TCP Packets/Bytes dropped before enqueue: "
				  << nTotalDroppedTcpPacketsBeforeEnqueue << " / "
				  << nTotalDroppedTcpBytesBeforeEnqueue
	 << std::endl << "UDP Packets/Bytes dropped before enqueue: "
				  << nTotalDroppedUdpPacketsBeforeEnqueue << " / "
				  << nTotalDroppedUdpBytesBeforeEnqueue;
...
TypeId QueueDisc::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::QueueDisc")
	...
	.AddTraceSource ("RredFilter", "Filter a packet by RRED filter",
					 MakeTraceSourceAccessor (&QueueDisc::m_traceRredFilter),
					 "ns3::QueueDiscItem::TracedCallback")
...
void
QueueDisc::RREDFilterBeforeEnqueue (Ptr<const QueueDiscItem> item)
{
  NS_LOG_FUNCTION (this << item);

  // Updates stats
  m_stats.nTotalFilteredPackets++;
  m_stats.nTotalFilteredBytes += item->GetSize ();
  ...
  Ptr<const Ipv4QueueDiscItem> ipItem = DynamicCast<const Ipv4QueueDiscItem>(item);
    ...
    if (prot == 6) // Update dropped TCP packets count
    {
  	  m_stats.nTotalFilteredTcpPackets ++;
  	  m_stats.nTotalFilteredTcpBytes += item->GetSize ();
    }
    if (prot == 17) // Update dropped UDP packets count
    {
  	  m_stats.nTotalFilteredUdpPackets ++;
  	  m_stats.nTotalFilteredUdpBytes += item->GetSize ();
    }

    NS_LOG_DEBUG ("Total packets/bytes filtered before enqueue: "
                  << m_stats.nTotalFilteredPackets << " / "
                  << m_stats.nTotalFilteredBytes);
    NS_LOG_LOGIC ("m_traceDropBeforeEnqueue (p)");
    m_traceRredFilter (item);
}
...
QueueDisc::DropBeforeEnqueue (Ptr<const QueueDiscItem> item, const char* reason)
{
  ...
  // Update the dropped packets stats
  Ptr<const Ipv4QueueDiscItem> ipItem = DynamicCast<const Ipv4QueueDiscItem>(item);
  Ipv4Header ipHeader = ipItem->GetHeader();
  uint8_t prot = ipHeader.GetProtocol();
  ...
  if (prot == 6) // Update dropped TCP packets count
  {
	  m_stats.nTotalDroppedTcpPacketsBeforeEnqueue ++;
	  m_stats.nTotalDroppedTcpBytesBeforeEnqueue += item->GetSize ();
  }
  if (prot == 17) // Update dropped UDP packets count
  {
	  m_stats.nTotalDroppedUdpPacketsBeforeEnqueue ++;
	  m_stats.nTotalDroppedUdpBytesBeforeEnqueue += item->GetSize ();
  }
...
QueueDisc::Enqueue (Ptr<QueueDiscItem> item)
{
  ...
  // Counting how the number of TCP and UDP packets received
  Ptr<const Ipv4QueueDiscItem> ipItem = DynamicCast<const Ipv4QueueDiscItem>(item);
  Ipv4Header ipHeader = ipItem->GetHeader();
  uint8_t prot = ipHeader.GetProtocol();
    ...
    if (prot == 6) // Update dropped TCP packets count
    {
  	  m_stats.nTotalReceivedTcpPackets ++;
  	  m_stats.nTotalReceivedTcpBytes += item->GetSize ();
    }
    if (prot == 17) // Update dropped UDP packets count
    {
  	  m_stats.nTotalReceivedUdpPackets ++;
  	  m_stats.nTotalReceivedUdpBytes += item->GetSize ();
    }


  bool retval = DoEnqueue (item);

  // if the item is successfully enqueued, update the time stamp to the actual enqueue time
  if (retval)
    {
      item->SetTimeStamp (Simulator::Now ());
    }
  ...
  NS_ASSERT (m_stats.nTotalReceivedPackets == m_stats.nTotalFilteredPackets + m_stats.nTotalDroppedPacketsBeforeEnqueue +
             m_stats.nTotalEnqueuedPackets);
  NS_ASSERT (m_stats.nTotalReceivedBytes == m_stats.nTotalFilteredBytes + m_stats.nTotalDroppedBytesBeforeEnqueue +
