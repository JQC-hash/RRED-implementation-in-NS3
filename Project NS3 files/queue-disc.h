...
public:
  ...
  struct Stats
  {
	  /// Total received packets
	      ...
	      uint32_t nTotalReceivedTcpPackets;
	      /// Total received TCP bytes
	      uint32_t nTotalReceivedTcpBytes;
	      /// Total received UDP packets
	      uint32_t nTotalReceivedUdpPackets;
	      /// Total received UDP bytes
	      uint32_t nTotalReceivedUdpBytes;
	      ...
	      uint32_t nTotalFilteredPackets;
	      /// Total filtered bytes
	      uint32_t nTotalFilteredBytes;
	      /// Total filtered TCP packets
	      uint32_t nTotalFilteredTcpPackets;
	      /// Total filtered TCP bytes
	      uint32_t nTotalFilteredTcpBytes;
	      /// Total filtered UDP packets
	      uint32_t nTotalFilteredUdpPackets;
	      /// Total filtered UDP bytes
	      uint32_t nTotalFilteredUdpBytes;
	      ...
	      /// Total TCP packets dropped before enqueue
	      uint32_t nTotalDroppedTcpPacketsBeforeEnqueue;
	      /// Total TCP bytes dropped before enqueue
	      uint64_t nTotalDroppedTcpBytesBeforeEnqueue;
	      /// Total UDP packets dropped before enqueue
	      uint32_t nTotalDroppedUdpPacketsBeforeEnqueue;
	      /// Total UDP bytes dropped before enqueue
	      uint64_t nTotalDroppedUdpBytesBeforeEnqueue;
...
protected:
  ...
  /**
   *  \brief Perform the actions required when the queue disc is notified of
   *         a packet filtered before enqueue by RRED filter
   *  \param item item that was filtered
   *  This method must be called by subclasses RED-Queue-Disc to record that a packet was
   *  filtered before enqueue by the RRED filter
   */
  void RREDFilterBeforeEnqueue (Ptr<const QueueDiscItem> item);
...
private:
  ...
  /// Traced callback: fired when a packet is filterd by RRED filter
  TracedCallback<Ptr<const QueueDiscItem> > m_traceRredFilter;
